          ...
          verbose = FALSE, w.init=NULL)
{
    ...
    fun <- match.arg(fun)
    n <- nrow(X)    # g - number of genes
    p <- ncol(X)    # N - number of samples
    ...
    if (method == "R") {
        ...
        X <- scale(X, scale = FALSE)   # centering to each N

        X <- if (row.norm) t(scale(X, scale=row.norm)) else t(X)    # N x g

        if (verbose) message("Whitening")
        # creating covariance matrix of X
        V <- X %*% t(X) / n       # N x g %*% g x N = N x N

        # SVD on covariance matrix
        s <- La.svd(V)
        # saving eigen values
        D <- diag(c(1/sqrt(s$d)))  # N x N

        # eigen values * eigen vectors
        K <- D %*% t(s$u)         # N x N %*% N x N = N x N

        # taking only k eigen vectors from K
        K <- matrix(K[1:n.comp, ], n.comp, p)  # k x N

        # reduced dataset X1 from original dataset X
        X1 <- K %*% X  # k x N %*%  N x g = k x g ?
