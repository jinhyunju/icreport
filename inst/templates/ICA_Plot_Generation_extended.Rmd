...
for( p in 1:dim(ica.result$A)[1]){
    ...
    if(is.null(ica.result$geneinfo)){
       ...
       # If IC is correlated to a covariate color the pot accordingly
        ...
        mclust.plots[[3]] <- ggplot(geneinfo.df, aes(x = chromosome_name, fill = chromosome_name)) + geom_bar() + 
                                     theme_bw() + theme(legend.position = "none") + xlab("Chromosome") + scale_x_discrete(drop=FALSE) +
        ...
        mclust.plots[[4]] <- ggplot(geneinfo.df, aes(x = gene_biotype, fill = gene_biotype)) + geom_histogram() + coord_flip() +
                                     ...
                                     scale_x_discrete(drop=FALSE,limits = rev(levels(geneinfo.df$gene_biotype)) ) +
