...
```{r Covariate_correlation_summary,echo= FALSE, comment=NA, fig.show = "hold", fig.width = 14,fig.height = 6 }
...
```{r Individual_IC_Plots, echo = FALSE,comment="",warning=FALSE,fig.width = 10,fig.height = 5}
for( p in 1:dim(ica.result$A)[1]){
    ...
    if(is.null(ica.result$geneinfo)){
       ...
       # Plot IC loading with peaks labeled in red (If peaks are available)
        ...
        mclust.plots[[4]] <- plot_component_hist(ica.result$S[,i],"Gene Weight Histogram") + coord_flip()
        ...
        # Need to figure out which plot might be most effective
            ...
            if(class(info.df[,color.by]) == "factor"){
                  mclust.plots[[3]] <- ggplot(coeff.plot.df, aes_string(x = color.by, y = "IC", color = color.by)) + 
                                              geom_boxplot() + xlab(paste("Covariate = ",color.by,sep="")) + ylab("") +  
                                              theme(axis.ticks = element_blank(), axis.text.x = element_blank()) 
            
            } else{
                  mclust.plots[[3]] <- ggplot(coeff.plot.df, aes_string(x = color.by, y = "IC", color = color.by)) + 
                                          geom_point() + xlab(paste("Covariate = ",color.by,sep=""))  + ylab(" ") +
                                          theme(axis.ticks = element_blank(), axis.text.x = element_blank())
            }
            ...
            mclust.plots[[3]] <- ggplot(coeff.plot.df, aes(x = mclust, y = IC, color = mclust)) + 
                                        geom_boxplot() + xlab(paste(mclust.obj$G,"clusters detected")) + ylab(" ") +  
                                        theme(axis.ticks = element_blank(), axis.text.x = element_blank())
            
            if(mclust.obj$G == 1){
                    mclust.plots[[2]] <- mclust.plots[[2]]
                    mclust.plots[[3]] <- mclust.plots[[3]]  + xlab("No clusters detected") + theme(legend.position = "none")
            }
