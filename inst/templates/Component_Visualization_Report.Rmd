...
if(method == "ica"){
  for( p in 1:dim(ica.result$A)[1]){
          ...
          if(!is.null(ica.result$ordered.geneinfo)){
            ...
            chromosomes <- unique(ica.result$ordered.geneinfo$pheno_chr)
            ...
            for(k in 1:length(chromosomes)){
              ...
              if(is.na(j)){
                ...
                x.axis[2,k] <- max(ica.result$ordered.geneinfo[,"idx"])
              } else {
                x.axis[1,k] <- min(subset(ica.result$ordered.geneinfo, pheno_chr == j)[,"idx"])
                x.axis[2,k] <- max(subset(ica.result$ordered.geneinfo, pheno_chr == j)[,"idx"])    
              }
...
if (method == "pca"){
  ...
  if(!is.null(pca.result$ordered.geneinfo)){
    ...
    # create axis tick positions for chromosomes if gene info is supplied
    chromosomes <- unique(pca.result$ordered.geneinfo$pheno_chr)
    x.axis <- matrix(0, nrow = 3, ncol = length(chromosomes))
    
    for(k in 1:length(chromosomes)){
      j <- chromosomes[k]
      x.axis[1,k] <- min(subset(pca.result$ordered.geneinfo, pheno_chr == j)[,"idx"])
      x.axis[2,k] <- max(subset(pca.result$ordered.geneinfo, pheno_chr == j)[,"idx"])
    }
  ...
  if(is.null(pca.result$ordered.geneinfo)){           
      ...
      mclust.plots[[1]] <- plot_component_chr(pca.result$rotation[,i], 
