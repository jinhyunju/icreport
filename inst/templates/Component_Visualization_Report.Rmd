  ...
  for( p in 1:dim(ica.result$A)[1]){
          ...
          if(!is.null(ica.result$geneinfo)){
            ...
            for(i in 1:length(chromosomes)){
              ...
              cat("level = ",i,"\n")
              match(i, chromosomes)
              x.axis[1,i] <- min(subset(ica.result$ordered.geneinfo, pheno_chr == j)[,"idx"])
              x.axis[2,i] <- max(subset(ica.result$ordered.geneinfo, pheno_chr == j)[,"idx"])
            }
         ...
         if(is.null(ica.result$ordered.geneinfo)){           
           if(!is.null(ica.result$peaks)){
            ...
            if(!is.null(ica.result$peaks)){
                # If the peaks are labeled use them 
                mclust.plots[[1]] <- plot_component_chr(ica.result$S[,i], 
                                                        geneinfo = ica.result$ordered.geneinfo,
                                                        x.axis = x.axis,
