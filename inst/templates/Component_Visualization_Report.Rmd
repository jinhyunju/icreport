...
if(method == "ica"){
         ...
         # If IC is correlated to a covariate color the pot accordingly
          if(i %in% unique(ica.result$cov.corr.idx$IC)){
             ...
             # Need to figure out which plot might be most effective
              ...
              mclust.plots[[2]] <- ggplot(coeff.plot.df, aes(x= idx, y = IC, color = mclust)) + 
                                          geom_point(size = 3, pch = 20) + 
                                          xlab("Sample idx") + ylab("IC coefficient") + labs(title = "Classification by Mclust") +  theme(legend.position = "none")
          }
  ...
  coeff.plot.df <- data.frame("IC" = pca.result$x[,i],"idx"=c(1:dim(pca.result$x)[1]))
   ...
   coeff.plot.df <- data.frame(as.data.frame(pca.result$info.df),"IC" = pca.result$x[,i],"idx"=c(1:dim(pca.result$x)[1]))
